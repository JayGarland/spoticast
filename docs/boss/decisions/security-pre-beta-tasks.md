...
- Commit `d45f923` added a startup `RuntimeError` that refuses to boot if the default is still set.
...
- Mitigated: `REDIRECT_URI` env var is set in production (Cloudflare Tunnel setup), which bypasses the Host-header path entirely.
...
| 3 | SessionMiddleware cookie hardening | HIGH | Done — commit d45f923 |
